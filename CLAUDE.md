...
- **Handoff** (D-040, D-041, `docs/17-handoff.md`) — `.agent/handoffs/`, gitignored. A bounded, inheritable context object for moving work between agents, in three levels: a derived project capsule, a session capsule holding the user's request verbatim, and packets in four modes. Inheritance is by reference, so one corrected capsule corrects every packet cut from it. The mode decides what a child sees of the parent's *judgement* — `worker` discloses it, `scout`/`reviewer`/`advisor` withhold it until a recorded `reveal` — while the facts the parent established are always included. Scope defaults to the whole project; results carry coverage so the next wave is delegated against evidence. Disposable coordination state, never a record, invisible to search, context and the compiler.
...
- **`crates/akr-cli`** — the `akr` binary, and a library (`akr_cli`) so the MCP server reuses the exact same command implementations. `handoff/` holds two projections that are not the ledger: `session_head.rs` (what `akr start` prepends) and `capsule.rs`/`packet.rs`/`result.rs`/`ops.rs`/`snapshot.rs` (the agent-to-agent context transport, its store, and the workspace fingerprint that tells a child whether the tree still matches what it was handed).
