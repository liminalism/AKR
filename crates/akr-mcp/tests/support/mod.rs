...
impl Example {
    ...
    pub fn run(&self, args: &[&str]) -> Run {
        let mut command = Command::new(akr_binary());
...
/// The `akr` binary, which this crate does not build.
///
/// `CARGO_BIN_EXE_*` is only set for a binary of the crate under test, and the differential
/// tests need the *other* crate's binary — that is the whole point of them. Cargo puts both
/// in the same target directory, so the mcp binary's own path locates it.
pub fn akr_binary() -> PathBuf {
    binary_beside("akr")
}
