...
| `AKR-M023` | legacy source path has no saved copy | warning | `source path {path} has no saved copy in sources/` | The link target exists but no immutable copy is registered; it becomes unverifiable the moment it moves. Register it with `akr source add`. |
